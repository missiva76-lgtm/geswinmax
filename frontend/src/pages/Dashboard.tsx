...
import { FileSpreadsheet, Package, RefreshCw, AlertCircle, CheckCircle, Clock, BarChart2, Archive, Database, ChevronDown, ChevronRight } from 'lucide-react'
...
export default function Dashboard() {
  // CORRIGIDO 28/07/2026: a lista mostrava apenas "erro", sem forma de saber
  // porquê — obrigava a ir buscar o log por outros meios de cada vez que algo
  // falhava. Agora cada job pode ser aberto para ver o log completo aqui mesmo.
  ...
  // CORRIGIDO 28/07/2026: só considerava jobs do tipo 'sync' (artigos), ignorando
  // Arquivo Digital e SAF-T, e usava `criado_em` (quando ARRANCOU) em vez da data de
  // conclusão. Como as sincronizações são agora agendadas automaticamente, este campo
  // tem de refletir a última que terminou, seja ela automática ou manual.
  const TIPOS_SYNC = ['sync', 'arquivo', 'saft']
  const instanteJob = (j: Job) => {
    const t = j.concluido_em || j.criado_em
    return (t as any)?.seconds ?? (t as any)?._seconds ?? 0
  }
  const ultimaSync = jobs
    .filter(j => TIPOS_SYNC.includes(j.tipo) && j.estado === 'concluido')
    .sort((a, b) => instanteJob(b) - instanteJob(a))[0]
    ...
    <div className="flex-1 overflow-auto p-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          ...
          { label: 'Última sync',      value: ultimaSync ? fmtDate(ultimaSync.concluido_em || ultimaSync.criado_em, { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' }) : '—', cor: '#7c3aed', bg: '#f5f3ff' },
        ].map((s, i) => (
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-5 py-3.5 border-b border-gray-50 flex items-center justify-between">
          ...
          <span className="text-xs text-gray-400">últimos {Math.min(jobs.length, 10)}</span>
        </div>
        ...
        {jobs.slice(0, 10).map((job) => (
          <div key={job.id} className="border-b border-gray-50 last:border-0">
            <button onClick={() => setJobAberto(jobAberto === job.id ? null : job.id)}
            ...
            {jobAberto === job.id && (
              <div className="px-5 pb-4">
                {job.erro_geral && (
                  <div className="mb-2 px-3 py-2 rounded-lg bg-red-50 border border-red-200 text-xs text-red-700 break-words">
                    <strong>Erro:</strong> {job.erro_geral}
                  </div>
                )}
                <div className="bg-gray-900 rounded-lg p-3 max-h-72 overflow-y-auto font-mono text-xs">
                  {(job.log && job.log.length > 0)
                    ...
                    : <div className="text-gray-500">(sem log registado para este job)</div>}
                </div>
              </div>
            )}
          </div>
